...
import { PowerAppService } from 'src/app/services/power-app.service';

@Component({
  selector: 'app-reincription',
  templateUrl: './reincription.component.html',
  styleUrls: ['./reincription.component.css']
})
export class ReincriptionComponent implements OnInit {
  user: any;

  constructor(private _route: ActivatedRoute, private router: Router, private formBuilder: FormBuilder, private _powerAppService: PowerAppService) {

    this._route.params.subscribe((params: Params) => {
      this.id = params.idSalesforce;
  ...
  ngOnInit() {
    ...
    var success = document.getElementById("btn-success");
    const { width } = volver.getBoundingClientRect();
    // alert(width);
    success.style.width = `${width}px`;
  }

  continuo(respuesta) {
    this.user.value.continua = respuesta;
    //console.log("Dijo que si : ", this.user.value);
    //.router.navigate(['www.google.com']);

    this._powerAppService.enviarConfirmacion(this.user.value).subscribe((response) => {
      console.log(response);
      window.location.replace('https://www.letralibre.cl/reinscripci%C3%B3n-confirmada');
    },
      (error) => {
        console.log(error);
  ...
  cancelo(respuesta) {
    this.user.value.continua = respuesta;
    //console.log("Dijo que no : ", this.user.value);
    this._powerAppService.enviarConfirmacion(this.user.value).subscribe((response) => {
      console.log(response);
      window.location.replace('https://www.letralibre.cl/reinscripci%C3%B3n-cancelada');
    },
      (error) => {
        console.log(error);
        alert("Ha ocurrido un error");
